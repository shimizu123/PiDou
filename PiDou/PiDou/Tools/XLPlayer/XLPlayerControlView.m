...
@interface XLPlayerControlView () <UIGestureRecognizerDelegate>
...
/**语音播放的背景图片*/
@property (nonatomic, strong) UIImageView *voiceBgImgV;
...
- (instancetype)init {
    self = [super init];
    if (self) {
        self.voiceBgImgV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"sound wave-white"]];
        [self addSubview:self.voiceBgImgV];
        self.voiceBgImgV.hidden = YES;
        
//        self.voiceImgV = [[YYAnimatedImageView alloc] initWithImage:[YYImage imageNamed:@"voiceRecording.gif"]];
...
- (void)makeSubViewsConstraints {
    ...
    [self.voiceBgImgV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.mas_left);
        make.right.equalTo(self.mas_right);
        make.centerY.equalTo(self.mas_centerY);
        make.height.mas_offset(60 * kWidthRatio6s);
    }];
    
    [self.voiceImgV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.centerY.equalTo(self.voiceBgImgV);
        make.width.mas_equalTo(self.voiceImgV.mas_height).multipliedBy(69 / 39.0);
    }];
    ...
    [self.lockBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(self.mas_leading).offset(15 + XL_LIUHAI_H);
        make.centerY.equalTo(self.mas_centerY);
...
- (void)playBtnClick:(UIButton *)sender {
    sender.selected = !sender.selected;
    [self voiceHidden:!sender.selected];
...
- (void)setOrientationLandscapeConstraint {
    ...
    [self.backBtn setImage:[UIImage imageNamed:@"back-white"] forState:(UIControlStateNormal)];
...
//        [_startBtn setImage:[UIImage imageNamed:@"video_play"] forState:UIControlStateNormal];
//        [_startBtn setImage:[UIImage imageNamed:@"video_pause"] forState:UIControlStateSelected];
...
//        [_fullScreenBtn setImage:ZFPlayerImage(@"ZFPlayer_shrinkscreen") forState:UIControlStateSelected];
        ...
        [_fullScreenBtn setImage:[UIImage imageNamed:@"CBNPlayer_MiniScreen"] forState:UIControlStateSelected];
...
- (void)setXl_showBackBtn:(BOOL)xl_showBackBtn {
    _xl_showBackBtn = xl_showBackBtn;
    self.backBtn.hidden = !_xl_showBackBtn;
}

- (void)setXl_isVoice:(BOOL)xl_isVoice {
    _xl_isVoice = xl_isVoice;
    //self.voiceImgV.hidden = !_xl_isVoice;
}

- (void)voiceHidden:(BOOL)isHidden {
    if (self.xl_isVoice) {
        self.voiceImgV.hidden = isHidden;
        self.voiceBgImgV.hidden = !isHidden;
    } else {
        self.voiceImgV.hidden = YES;
    }
}

- (void)setXl_state:(XLPlayerState)xl_state {
    _xl_state = xl_state;
    switch (_xl_state) {
        case XLPlayerStateFailed:
        {
            [self voiceHidden:YES];
        }
            break;
        case XLPlayerStateBuffering:
        {
            [self voiceHidden:YES];
        }
            break;
        case XLPlayerStatePlaying:
        {
            [self voiceHidden:NO];
        }
            break;
        case XLPlayerStateStopped:
        {
            [self voiceHidden:YES];
        }
            break;
        case XLPlayerStatePause:
        {
            [self voiceHidden:YES];
        }
            break;
