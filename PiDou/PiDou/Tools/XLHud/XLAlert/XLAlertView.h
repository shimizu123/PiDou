 ...
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setCustomSuperView)(UIView *customSuperView);
...
@property (nonatomic, copy, readonly) XLAlertView *(^setTextViewUserInteractionEnabled)(BOOL userInteractionEnabled);

/** title和message是否可以选择文字，默认NO */
@property (nonatomic, assign) BOOL textViewCanSelectText;

/** 设置title和message是否可以选择文字，默认NO */
@property (nonatomic, copy, readonly) XLAlertView *(^setTextViewCanSelectText)(BOOL canSelectText);

/**
 * 设置titleTextColor
 * plain默认RGB都为0.1，其它0.35
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTitleTextColor)(UIColor *textColor);

/**
 * 设置titleTextFont
 * plain默认 bold 17，其它17
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTitleTextFont)(UIFont *font);

/**
 * 设置messageTextColor
 * plain默认RGB都为0.55，其它0.3
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setMessageTextColor)(UIColor *textColor);
 ...
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setMessageTextFont)(UIFont *font);

/** titleTextViewDelegate */
@property (nonatomic, weak) id<UITextViewDelegate> titleTextViewDelegate;

/** 设置titleTextViewDelegate */
@property (nonatomic, copy, readonly) XLAlertView *(^setTitleTextViewDelegate)(id<UITextViewDelegate> delegate);

/** messageTextViewDelegate */
@property (nonatomic, weak) id<UITextViewDelegate> messageTextViewDelegate;

/** 设置messageTextViewDelegate */
@property (nonatomic, copy, readonly) XLAlertView *(^setMessageTextViewDelegate)(id<UITextViewDelegate> delegate);
...
@property (nonatomic, copy, readonly) XLAlertView *(^setTitleTextViewAlignment)(NSTextAlignment textAlignment);
...
@property (nonatomic, copy, readonly) XLAlertView *(^setMessageTextViewAlignment)(NSTextAlignment textAlignment);

/** title和message的左右间距 默认20 */
@property (nonatomic, assign) CGFloat textViewLeftRightMargin;

/** 设置title和message的左右间距 默认20 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTextViewLeftRightMargin)(CGFloat margin);
 ...
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTextViewTopBottomMargin)(CGFloat margin);

/** 默认的取消action，不需要自带的可以自己设置，不可置为nil */
@property (nonatomic, strong) XLAlertAction *cancelAction;

/** 设置默认的取消action，不需要自带的可以自己设置，不可置为nil */
@property (nonatomic, copy, readonly) XLAlertView *(^setCancelAction)(XLAlertAction *action);
...
@property (nonatomic, copy, readonly) XLAlertView *(^enableDeallocLog)(BOOL enable);

/**
 * 设置背景view
 * 默认是一个UIToolbar，背景是黑色0.1透明度
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setBackGroundView)(UIView *(^backGroundView)(void));
 ...
 * 不可小于0，不可大于屏幕宽度
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setPlainWidth)(CGFloat width);
 ...
 */
@property (nonatomic, copy, readonly) XLAlertView *(^addCustomPlainTitleView)(UIView *(^customView)(void));

/** 设置title和message之间的间距 默认7 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTitleMessageMargin)(CGFloat margin);
 ...
 * 正数表示向下偏移，负数表示向上偏移
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setPlainCenterOffsetY)(CGFloat centerOffsetY, BOOL animated);
 ...
 * 小于等于0表示不自动隐藏
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setDismissTimeInterval)(CGFloat dismissTimeInterval);
 ...
 * 正数表示向下偏移，负数表示向上偏移
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setHUDCenterOffsetY)(CGFloat centerOffsetY);
 ...
 * 小于0将没有效果，默认-1
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setHUDHeight)(CGFloat height);
 ...
 * 默认的高度是宽度-6，暂不支持自定义高度
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setFlowlayoutItemWidth)(CGFloat width);
 ...
 * 设置YES时会自动让两个collection的action数量保持一致，即向少的一方添加空的action
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setCompoundCollection)(BOOL compoundCollection);

/** collection是否分页 */
@property (nonatomic, assign) BOOL collectionPagingEnabled;

/** 设置collection是否分页 */
@property (nonatomic, copy, readonly) XLAlertView *(^setCollectionPagingEnabled)(BOOL collectionPagingEnabled);
 ...
 * XLAlertAction使用类方法初始化时每个参数传nil或者直接自己实例化一个即为空action
 */
@property (nonatomic, copy, readonly) XLAlertView *(^setShowPageControl)(BOOL showPageControl);

/** colletion样式的底部按钮左右间距 */
@property (nonatomic, assign) CGFloat collectionButtonLeftRightMargin;

/** 设置colletion样式的底部按钮左右间距 */
@property (nonatomic, copy, readonly) XLAlertView *(^setCollectionButtonLeftRightMargin)(CGFloat margin);

/** collection样式默认有一个取消按钮，设置这个可以在取消按钮的上面再添加一个按钮 */
@property (nonatomic, strong) XLAlertAction *collectionAction;

/** collection样式默认有一个取消按钮，设置这个可以在取消按钮的上面再添加一个按钮 */
@property (nonatomic, copy, readonly) XLAlertView *(^setCollectionAction)(XLAlertAction *action);
 ...
 */
@property (nonatomic, copy, readonly) XLAlertView *(^addCustomCollectionTitleView)(UIView *(^customView)(void));

/** 添加第二个collectionView的action */
- (void)addSecondCollectionAction:(XLAlertAction *)action;

/** 链式添加第二个collectionView的action */
@property (nonatomic, copy, readonly) XLAlertView *(^addSecondCollectionAction)(XLAlertAction *action);
...
/** 链式添加action */
@property (nonatomic, copy, readonly) XLAlertView *(^addAction)(XLAlertAction *action);
...
/** 链式添加textField */
@property (nonatomic, copy, readonly) XLAlertView *(^addTextFieldWithConfigurationHandler)(void (^ __nullable)(UITextField *textField));


#pragma mark - 显示

/** 显示 */
@property (nonatomic, copy, readonly) id<XLAlertViewProtocol> (^show)(void);

/** 监听显示动画完成 */
@property (nonatomic, copy, readonly) id<XLAlertViewProtocol> (^setShowAnimationComplete)(void(^showAnimationComplete)(XLAlertView *view));
...
@property (nonatomic, copy, readonly) XLAlertView *(^setBottomButtonMargin)(CGFloat margin);

/** 设置是否自动适配 iPhone X homeIndicator 默认YES */
@property (nonatomic, copy, readonly) XLAlertView *(^setAutoAdjustHomeIndicator)(BOOL autoAdjust);


#pragma mark - 旋转适配
/** 是否是动画旋转，默认false */
@property (nonatomic, assign) BOOL isTransformRotation;
/** 设置dealloc时会调用的block */
/** 设置是否允许dealloc打印，用于检查循环引用 */
@property (nonatomic, copy, readonly) XLAlertView *(^setTransformRotation)(BOOL isTransform);
