...
#define kPageControl_H   10
static NSString * XLCircleCellID = @"kXLCircleCell";

@interface XLCircleView () <UICollectionViewDelegate,UICollectionViewDataSource> {
    NSInteger _index;
    BOOL _isScrol;
...
@property (nonatomic, strong) XLCirclePageControl *pageControl;
@property (nonatomic, strong) NSTimer *timer;

@end

@implementation XLCircleView
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    XLCircleCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:XLCircleCellID forIndexPath:indexPath];
...
- (void)startTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:(self.timeInterval ? self.timeInterval : 3) target:self selector:@selector(timerCycleImageAction:) userInfo:nil repeats:YES];
...
#pragma mark - getter
- (XLCirclePageControl *)pageControl {
    if (!_pageControl) {
        
        _pageControl = [[XLCirclePageControl alloc]initWithFrame:CGRectMake(0, self.bounds.size.height - kPageControl_H, self.bounds.size.width, kPageControl_H)];
        _pageControl.currentPage = 0;
        _pageControl.currentPageIndicatorTintColor = XL_COLOR_RED;
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[XLCircleCell class] forCellWithReuseIdentifier:XLCircleCellID];
