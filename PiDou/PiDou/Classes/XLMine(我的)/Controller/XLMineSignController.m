...
@interface XLMineSignController () <UITextFieldDelegate>

@property (nonatomic, strong) XLLimitTextField *detailTF;

@end

@implementation XLMineSignController
...
- (void)initUI {
    ...
    self.detailTF.xl_placeholder = @"15个字以内";
    self.detailTF.text = self.sign;
...
- (void)saveAction {
    NSString *sign = self.detailTF.text;
    if (XLStringIsEmpty(sign)) {
        [HUDController hideHUDWithText:@"请输入签名"];
        return;
    }
    [HUDController xl_showHUD];
    [XLMineHandle userInfoUpdateWithNickname:@"" sign:sign sex:@"" avatar:@"" success:^(id  _Nonnull responseObject) {
