...
@property (nonatomic, strong) XLMainDetailTable *table;
...
@property (nonatomic, strong) NSMutableArray *data;// 评论信息

/**对当前选中的评论进行回复的当前评论model*/
@property (nonatomic, strong) XLCommentModel *currentReplyComment;

@end

@implementation XLMainDetailController
...
- (void)initCommentData {
    kDefineWeakSelf;
    [XLCommentHandle commentListWithEntity_id:self.entity_id page:self.page success:^(NSMutableArray *responseObject) {
        if (WeakSelf.page > 1) {
        ...
        WeakSelf.table.commentsData = WeakSelf.data;
...
- (void)publish {
    ...
    [XLCommentHandle postCommentWithEntity_id:self.entity_id cid:self.currentReplyComment.cid rid:nil data:paramDic.mj_JSONString success:^(id  _Nonnull responseObject) {
        [HUDController hideHUD];
...
#pragma mark - XLMainDetailTableDelegate
...
#pragma mark - lazy load
- (XLMainDetailTable *)table {
    if (!_table) {
        _table = [[XLMainDetailTable alloc] init];
        _table.delegate = self;
        _table.mainType = self.mainType;
    }
    return _table;
}
