...
#define XLSPACING 6 * kWidthRatio6s

@interface XLPhotoViewController ()
...
XLPhotoViewCellDelegate,
XLPhotoBottomViewDelegate,
...
@property (weak, nonatomic) XLPhotoViewSectionFooterView *footerView;
...
@property (nonatomic, strong) XLPhotoTopView *photoTopView;

@end

@implementation XLPhotoViewController

#pragma mark - < life cycle >
- (void)dealloc {
    if (HXShowLog) NSSLog(@"dealloc");
    [self.manager removeAllTempList];
    if (self.manager.configuration.albumShowMode == HXPhotoAlbumShowModePopup || self.manager.configuration.albumShowMode == HXPhotoAlbumShowModeIceVideo) {
...
- (void)viewDidLoad {
    ...
    if (self.manager.configuration.albumShowMode == HXPhotoAlbumShowModeDefault) {
        ...
        [self getPhotoList];
    }else if (self.manager.configuration.albumShowMode == HXPhotoAlbumShowModePopup || self.manager.configuration.albumShowMode == HXPhotoAlbumShowModeIceVideo) {
        [self setupUI];
        [self changeSubviewFrame];
        // 获取当前应用对照片的访问授权状态
        HXWeakSelf
...
#pragma mark - < private >
- (void)setupUI {
    self.photoTopView = [[XLPhotoTopView alloc] initWithFrame:(CGRectMake(0, 0, SCREEN_WIDTH, 397 * kWidthRatio6s + XL_STATUS_H))];
    [self.view addSubview:self.photoTopView];
    self.photoTopView.delegate = self;
...
- (void)changeSubviewFrame {
    ...
    CGFloat itemWidth = (width - (lineCount - 1) - 4 * XLSPACING) / lineCount;
...
- (void)didCancelClick {
    ...
    if ([self.delegate respondsToSelector:@selector(xl_photoViewControllerDidCancel:)]) {
        [self.delegate xl_photoViewControllerDidCancel:self];
    }
...
- (XLPhotoViewCell *)currentPreviewCell:(HXPhotoModel *)model {
    ...
    return (XLPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
}
...
- (void)customCameraViewController:(HXCustomCameraViewController *)viewController didDone:(HXPhotoModel *)model {
    ...
    self.collectionView.contentInset = UIEdgeInsetsMake(0, 0, bottomViewH, 0);
    self.bottomView.frame = CGRectMake(0, CGRectGetMaxY(self.collectionView.frame) - bottomViewH - XL_NAVIBAR_H, CGRectGetWidth(self.bottomView.frame), bottomViewH);
    ...
    if (!self.manager.configuration.singleSelected) {
        ...
        self.photoTopView.model = model;
        [self.photoTopView cancelRequest];
        [self.photoTopView requestHDImage];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        XLPhotoCameraViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"DateCameraCellId" forIndexPath:indexPath];
        cell.model = model;
        ...
        XLPhotoViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"DateCellId" forIndexPath:indexPath];
        cell.delegate = self;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (granted) {
                    ...
                    XLPhotoCameraViewCell *cameraCell = (XLPhotoCameraViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
        ...
        XLPhotoViewCell *cell = (XLPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
        [self xl_photoViewCell:cell didSelectBtn:cell.selectBtn];
        return;
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[XLPhotoViewCell class]]) {
        [(XLPhotoViewCell *)cell cancelRequest];
    }
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    if ([kind isEqualToString:UICollectionElementKindSectionHeader] && self.manager.configuration.showDateSectionHeader) {
        XLPhotoViewSectionHeaderView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"sectionHeaderId" forIndexPath:indexPath];
        ...
        if (self.manager.configuration.showBottomPhotoDetail) {
            XLPhotoViewSectionFooterView *footerView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"sectionFooterId" forIndexPath:indexPath];
...
- (UIViewController *)previewingContext:(id<UIViewControllerPreviewing>)previewingContext viewControllerForLocation:(CGPoint)location {
    ...
    if (![[self.collectionView cellForItemAtIndexPath:indexPath] isKindOfClass:[XLPhotoViewCell class]]) {
        return nil;
    }
    XLPhotoViewCell *cell = (XLPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    ...
    vc.downloadImageComplete = ^(HXPhoto3DTouchViewController *vc, HXPhotoModel *model) {
        if (!model.loadOriginalImage) {
            XLPhotoViewCell *myCell = (XLPhotoViewCell *)[weakSelf.collectionView cellForItemAtIndexPath:vc.indexPath];
            if (myCell) {
...
- (void)previewingContext:(id<UIViewControllerPreviewing>)previewingContext commitViewController:(UIViewController *)viewControllerToCommit {
    ...
    XLPhotoViewCell *cell = (XLPhotoViewCell *)[self.collectionView cellForItemAtIndexPath:vc.indexPath];
    if (!self.manager.configuration.singleSelected) {
        HXPhotoPreviewViewController *previewVC = [[HXPhotoPreviewViewController alloc] init];
        if (HX_IOS9Earlier) {
            previewVC.photoViewController = self;
...
#pragma mark - < XLPhotoViewCellDelegate >
...
- (void)xl_photoViewCell:(XLPhotoViewCell *)cell didSelectBtn:(UIButton *)selectBtn {
    if (selectBtn.selected) {
        ...
        self.photoTopView.model = cell.model;
        [self.photoTopView cancelRequest];
        [self.photoTopView requestHDImage];
    }
    UIColor *bgColor;
    ...
    if (self.manager.videoSelectedType == HXPhotoManagerVideoSelectedTypeSingle) {
        for (UICollectionViewCell *tempCell in self.collectionView.visibleCells) {
            if ([tempCell isKindOfClass:[XLPhotoViewCell class]]) {
                if ([(XLPhotoViewCell *)tempCell model].subType == HXPhotoModelMediaSubTypeVideo) {
    ...
    self.bottomView.selectCount = [self.manager selectedCount];
    if ([self.delegate respondsToSelector:@selector(xl_photoViewControllerDidChangeSelect:selected:)]) {
        [self.delegate xl_photoViewControllerDidChangeSelect:cell.model selected:selectBtn.selected];
    }
...
- (void)photoPreviewControllerDidSelect:(HXPhotoPreviewViewController *)previewController model:(HXPhotoModel *)model {
    ...
    if (self.manager.videoSelectedType == HXPhotoManagerVideoSelectedTypeSingle) {
        for (UICollectionViewCell *tempCell in self.collectionView.visibleCells) {
            if ([tempCell isKindOfClass:[XLPhotoViewCell class]]) {
                if ([(XLPhotoViewCell *)tempCell model].subType == HXPhotoModelMediaSubTypeVideo &&
                    [(XLPhotoViewCell *)tempCell model] != model) {
                    [indexPathList addObject:[self.collectionView indexPathForCell:tempCell]];
                }
            }
        }
    ...
    self.bottomView.selectCount = [self.manager selectedCount];
    if ([self.delegate respondsToSelector:@selector(xl_photoViewControllerDidChangeSelect:selected:)]) {
        [self.delegate xl_photoViewControllerDidChangeSelect:model selected:model.selected];
    }
}
...
#pragma mark - < XLPhotoBottomViewDelegate >
...
- (void)xl_photoBottomViewDidEditBtn {
    ...
    if (model.type == HXPhotoModelMediaTypePhotoGif ||
        ...
        hx_showAlert(self, [NSBundle hx_localizedStringForKey:@"编辑后，GIF将会变为静态图，确定继续吗？"], nil, [NSBundle hx_localizedStringForKey:@"取消"], [NSBundle hx_localizedStringForKey:@"确定"], nil, ^{
            [weakSelf xl_jumpEditViewControllerWithModel:model];
        });
        return;
    }
    ...
    [self xl_jumpEditViewControllerWithModel:model];
}
- (void)xl_jumpEditViewControllerWithModel:(HXPhotoModel *)model {
    if (model.subType == HXPhotoModelMediaSubTypePhoto) {
...
- (void)xl_cleanSelectedList {
    ...
    if ([self.delegate respondsToSelector:@selector(xl_photoViewController:didDoneAllList:photos:videos:original:)]) {
        [self.delegate xl_photoViewController:self
...
- (XLPhotoBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[XLPhotoBottomView alloc] initWithFrame:CGRectMake(0, self.view.hx_h - hxBottomMargin, self.view.hx_w, hxBottomMargin)];
...
- (HXPhotoViewFlowLayout *)customLayout {
    if (!_customLayout) {
        ...
        _customLayout.minimumLineSpacing = XLSPACING;
        _customLayout.minimumInteritemSpacing = XLSPACING;
        _customLayout.sectionInset = UIEdgeInsetsMake(XLSPACING, XLSPACING, XLSPACING, XLSPACING);
        //        if (HX_IOS9Later) {
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        CGFloat top = CGRectGetMaxY(self.photoTopView.frame);
        CGFloat collectionHeight = self.view.hx_h - top;
        if (self.manager.configuration.showDateSectionHeader) {
            _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, top, self.view.hx_w, collectionHeight) collectionViewLayout:self.customLayout];
        }else {
            _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, top, self.view.hx_w, collectionHeight) collectionViewLayout:self.flowLayout];
        }
//        _collectionView.backgroundColor = [UIColor whiteColor];
        ...
        [_collectionView registerClass:[XLPhotoViewCell class] forCellWithReuseIdentifier:@"DateCellId"];
        [_collectionView registerClass:[XLPhotoCameraViewCell class] forCellWithReuseIdentifier:@"DateCameraCellId"];
        [_collectionView registerClass:[XLPhotoViewSectionHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"sectionHeaderId"];
        [_collectionView registerClass:[XLPhotoViewSectionFooterView class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"sectionFooterId"];
    ...
    - (UICollectionViewFlowLayout *)flowLayout {
        if (!_flowLayout) {
            ...
            _flowLayout.minimumLineSpacing = XLSPACING;
            _flowLayout.minimumInteritemSpacing = XLSPACING;
            _flowLayout.sectionInset = UIEdgeInsetsMake(XLSPACING, XLSPACING, XLSPACING, XLSPACING);
            //        if (HX_IOS9Later) {
    ...
    @end
    @interface XLPhotoCameraViewCell ()
    ...
    @implementation XLPhotoCameraViewCell
...
//    @property (strong, nonatomic) UIButton *selectBtn;
    ...
    @implementation XLPhotoViewCell
        ...
        if (self.model.type == HXPhotoModelMediaTypeVideo) {
            self.iCloudRequestID = [self.model requestAVAssetStartRequestICloud:^(PHImageRequestID iCloudRequestId, HXPhotoModel *model) {
                ...
                if (weakSelf.model == model) {
                    ...
                    if ([weakSelf.delegate respondsToSelector:@selector(xl_photoViewCellRequestICloudAssetComplete:)]) {
                        [weakSelf.delegate xl_photoViewCellRequestICloudAssetComplete:weakSelf];
                    }
                }
    ...
    - (void)didSelectClick:(UIButton *)button {
        ...
        if ([self.delegate respondsToSelector:@selector(xl_photoViewCell:didSelectBtn:)]) {
            [self.delegate xl_photoViewCell:self didSelectBtn:button];
        }
    }
...
#pragma mark - < 懒加载 >
    ...
    - (UIImageView *)imageView {
        if (!_imageView) {
            ...
            XLViewRadius(_imageView, 4 * kWidthRatio6s);
        }
        return _imageView;
    }
    - (UIView *)maskView {
    ...
    @interface XLPhotoViewSectionHeaderView ()
    ...
    @implementation XLPhotoViewSectionHeaderView
    - (instancetype)initWithFrame:(CGRect)frame {
        self = [super initWithFrame:frame];
        if (self) {
            [self setupUI];
    ...
    @interface XLPhotoViewSectionFooterView ()
    @property (strong, nonatomic) UILabel *titleLb;
    @end
    
    @implementation XLPhotoViewSectionFooterView
    - (instancetype)initWithFrame:(CGRect)frame {
        self = [super initWithFrame:frame];
        if (self) {
            [self setupUI];
    ...
    - (void)didDoneBtnClick {
        if ([self.delegate respondsToSelector:@selector(xl_photoBottomViewDidDoneBtn)]) {
            [self.delegate xl_photoBottomViewDidDoneBtn];
        }
    }
    - (void)didPreviewClick {
        if ([self.delegate respondsToSelector:@selector(xl_photoBottomViewDidPreviewBtn)]) {
            [self.delegate xl_photoBottomViewDidPreviewBtn];
        }
    }
    - (void)didEditBtnClick {
        if ([self.delegate respondsToSelector:@selector(xl_photoBottomViewDidEditBtn)]) {
            [self.delegate xl_photoBottomViewDidEditBtn];
        }
    }
