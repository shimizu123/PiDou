...
- (CGRect)getVideoRect {
    ...
    CGFloat videoW = self.view.hx_w - hxValidPlayerRectX * 2;
    ...
    return CGRectMake(hxValidPlayerRectX + (videoW - w) / 2, hxVideoY + (videoH - h) / 2, w, h);
}
...
- (void)didCloseWithPhotoNaviBar:(XLPhotoNaviBar *)photoNaviBar {
    [self videoEditBottomViewDidCancelClick:self.bottomView];
}
- (void)didNextWithPhotoNaviBar:(XLPhotoNaviBar *)photoNaviBar {
    [self videoEditBottomViewDidDoneClick:self.bottomView];
}
...
- (XLPhotoNaviBar *)naviBar {
    if (!_naviBar) {
        _naviBar = [[XLPhotoNaviBar alloc] initWithFrame:(CGRectMake(0, 0, SCREEN_WIDTH, XL_NAVIBAR_H))];
