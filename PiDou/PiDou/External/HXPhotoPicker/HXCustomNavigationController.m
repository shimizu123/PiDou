...
#import "XLAlbumListViewController.h"   // ice
#import "XLPhotoViewController.h"

@interface HXCustomNavigationController ()<HXAlbumListViewControllerDelegate, HXPhotoViewControllerDelegate, XLAlbumListViewControllerDelegate, XLPhotoViewControllerDelegate>
...
- (instancetype)initWithManager:(HXPhotoManager *)manager
    ...
    } else if (manager.configuration.albumShowMode == HXPhotoAlbumShowModeIceVideo) {
//        XLAlbumListViewController *vc = [[XLAlbumListViewController alloc] initWithManager:manager];
//        self = [super initWithRootViewController:vc];
//        if (self) {
//            self.hx_delegate = delegate;
//            self.manager = manager;
...
//            vc.delegate = self;
//        }
        XLPhotoViewController *vc = [[XLPhotoViewController alloc] init];
        vc.manager = manager;
        self = [super initWithRootViewController:vc];
        if (self) {
            self.hx_delegate = delegate;
...
#pragma mark - XLAlbumListViewControllerDelegate
...
- (void)xl_albumListViewController:(XLAlbumListViewController *)albumListViewController didDoneAllList:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photoList videos:(NSArray<HXPhotoModel *> *)videoList original:(BOOL)original {
    if ([self.hx_delegate respondsToSelector:@selector(photoNavigationViewController:didDoneAllList:photos:videos:original:)]) {
        [self.hx_delegate photoNavigationViewController:self didDoneAllList:allList photos:photoList videos:videoList original:original];
    }
}
...
#pragma mark - XLPhotoViewControllerDelegate
- (void)xl_photoViewControllerDidCancel:(XLPhotoViewController *)photoViewController {
    if ([self.hx_delegate respondsToSelector:@selector(photoNavigationViewControllerDidCancel:)]) {
        [self.hx_delegate photoNavigationViewControllerDidCancel:self];
    }
}
- (void)xl_photoViewController:(XLPhotoViewController *)photoViewController didDoneAllList:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photoList videos:(NSArray<HXPhotoModel *> *)videoList original:(BOOL)original {
    if ([self.hx_delegate respondsToSelector:@selector(photoNavigationViewController:didDoneAllList:photos:videos:original:)]) {
        [self.hx_delegate photoNavigationViewController:self didDoneAllList:allList photos:photoList videos:videoList original:original];
    }
}
